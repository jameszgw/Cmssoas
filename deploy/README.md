...
├── codeman-backend.env.example   # 后端环境变量样例(systemd 裸机部署用)
├── install.sh                    # CentOS 一键安装脚本(systemd + nginx)
├── systemd/codeman-backend.service
├── nginx/codeman.conf
...
```bash
# PostgreSQL
psql -U codeman -d codeman -f deploy/sql/schema-postgresql.sql
psql -U codeman -d codeman -f deploy/sql/init-admin.sql        # 可选(应用首启亦会做)
psql -U codeman -d codeman -f deploy/sql/test-data.sql         # 可选(演示数据)
# MySQL(库须 utf8mb4)
mysql -ucodeman -p codeman < deploy/sql/schema-mysql.sql
mysql -ucodeman -p codeman < deploy/sql/init-admin.sql
mysql -ucodeman -p codeman < deploy/sql/test-data.sql
```
