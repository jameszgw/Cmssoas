/**
 * CmRuleEngine × CODEMAN License 集成适配器(零依赖,浏览器 / Node ≥ 20 通用)。
 *
 * 消费方为 CmRuleEngine 仓库的 rule-engine-server:其通过环境变量
 *   RE_LICENSE=<.lic 文件路径>  与  RE_LICENSE_PUBKEY=<公钥Base64(SPKI)>
 * 直接读取并验签 License(见 rule-engine-server/src/capabilities.js)。
 * 本适配器供浏览器 / Node 宿主(如管理台、网关、二次封装服务)自行验签与能力解析:
 *
 *   import { resolve } from 'cmrule/capabilities'            // rule-engine-server 同款解析
 *   import { loadCmruleLicense } from './cmrule-license.mjs'
 *
 *   const { edition, overrides } = await loadCmruleLicense(licText, publicKeyBase64)
 *   const capabilities = resolve(edition, overrides)          // ← 喂给规则引擎能力门禁
 *
 * 契约:claims.edition ∈ COMMUNITY/PROFESSIONAL/ENTERPRISE/ULTIMATE;
 * claims.features 是签发时固化的「档位预设 ∪ 合同微调」能力表(键名=CAPABILITY_KEYS 19 键)——
 * 作为 overrides 显式传入,即使 CmRuleEngine 侧 PRESETS 预设日后调整,已签发 License 的能力门禁也不漂移。
 *
...
export function validateClaims(claims, now = new Date()) {
  if (claims.productCode !== PRODUCT_CODE) {
    return { ok: false, reason: `非 CmRuleEngine License(productCode=${claims.productCode})` }
  }
...
/** claims → CmRuleEngine resolve(edition, overrides) 的入参(edition 保持大写,与 PRESETS 键一致)。 */
export function toCapabilities(claims) {
  return {
    edition: String(claims.edition || 'COMMUNITY').toUpperCase(),
 ...
 */
export async function loadCmruleLicense(licText, publicKeyBase64, { now } = {}) {
