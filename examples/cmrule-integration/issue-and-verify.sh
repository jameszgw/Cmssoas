#!/usr/bin/env bash
# CmRuleEngine 商业授权全链路脚本:登录 → 按档位签发(可带能力微调)→ 下载 .lic 与公钥 → 平台验签 →
# 本地验签 → 提示如何配置 rule-engine-server → 审计查询。
# 用法:
#   BASE_URL=http://localhost:8080 ADMIN_USER=admin ADMIN_PASS=8888 \
#   bash issue-and-verify.sh [edition] [customer] ['{"versionDiff":true}']
...
CUSTOMER="${2:-CmRuleEngine 集成演示客户}"
OVERRIDES="${3:-{\}}"
...
echo "== 2) 档位矩阵(/api/cmrule/editions)=="
curl -sf "${AUTH[@]}" "$BASE_URL/api/cmrule/editions" | python3 -c '
import sys, json
...
LIC_ID=$(curl -sf "${AUTH[@]}" "$BASE_URL/api/cmrule/licenses/issue" -d "{
  \"tenantCode\":\"T-CMR-DEMO\",\"customer\":\"$CUSTOMER\",\"edition\":\"$EDITION\",
...
echo "== 7) rule-engine-server 配置提示 =="
echo "  在 CmRuleEngine 部署侧设置环境变量,让 rule-engine-server 启动时验签并解析能力:"
echo "    export RE_LICENSE=$(pwd)/$LIC_ID.lic"
echo "    export RE_LICENSE_PUBKEY=$PUB"

echo "== 8) 审计查询(/api/cmrule/audit)=="
curl -sf "${AUTH[@]}" "$BASE_URL/api/cmrule/audit?keyword=$LIC_ID" | python3 -c '
import sys, json
v = json.load(sys.stdin)
print("  共 %d 条" % v["total"])
for r in v["rows"]:
...
echo "完成。集成端:rule-engine-server 经 RE_LICENSE/RE_LICENSE_PUBKEY 验签 → resolve(claims.edition, claims.features) 即得能力门禁"
