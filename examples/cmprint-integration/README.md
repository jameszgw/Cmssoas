...
- `issue-and-verify.sh` — 全链路脚本:登录 → 档位矩阵 → 签发(可带能力微调)→ 下载 .lic →
  平台验签 → 本地验签 → 审计查询。
- `cmprint-online-client.mjs` — **在线浮动席位客户端**(浏览器/Node):激活占座 → 自动心跳
  (nonce 防重放,Ed25519 响应验签)→ 关闭释放(pagehide sendBeacon);席位满抛
  `err.status==='SEAT_FULL'`;吊销/到期下一次心跳即时生效并回调 `onStatus`;断网进入宽限,
  恢复后自动重激活续座。实例标识默认按 licenseId 持久(同浏览器一座),`instanceId:
  crypto.randomUUID()` 可改为每标签页一座。
- `online-demo.mjs` — 在线席位全链路演示:`BASE_URL=... node online-demo.mjs`
  (签发并发2 → A/B 占满 → C 被拒 → 心跳验签 → B 释放 C 接替 → 吊销即时 REVOKED)。

## 在线浮动席位(ONLINE/HYBRID 模式)

```js
import { createOnlineClient } from './cmprint-online-client.mjs'

const client = createOnlineClient({
  baseUrl: PLATFORM, licenseId: claims.licenseId, publicKeyBase64: pub,
  onStatus(ev) {
    if (ev.type === 'revoked' || ev.type === 'expired') lockDesigner()   // 即时锁功能
    if (ev.type === 'seat-full') showSeatFullDialog(ev.message)
  },
})
await client.start()    // 激活并自动心跳;离线验签(loadCmprintLicense)+在线席位可叠加使用
```

平台侧 `/sdk/**` 与 `/pub/**` 已对任意来源放开 CORS(无凭据,鉴权靠 licenseId/galleryKey
+ 服务端校验),浏览器客户端可从集成方域名直调;心跳间隔/宽限期由服务端下发
(`app.online.heartbeat-sec` / `grace-sec`)。
