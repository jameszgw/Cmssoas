...
import {
  getCmruleEditions, listCmruleLicenses, issueCmruleLicense, queryCmruleAudit,
  type CmruleEditionsView, type CmruleAuditRow,
} from '@/api/cmrule'
...
// ---------- 档位矩阵 ----------
const editions = ref<CmruleEditionsView | null>(null)
/** 矩阵只展示四档有差异的能力键(任一档预设里出现过的键);其余键四档全开。 */
...
async function load() {
  loading.value = true
  try { rows.value = await listCmruleLicenses() }
  catch { ElMessage.error(t('cmrule.loadFail')) }
  finally { loading.value = false }
}

onMounted(async () => {
  try { editions.value = await getCmruleEditions() } catch { /* 矩阵加载失败不阻塞列表 */ }
  await load()
  await loadAudit()
})
...
const EDITION_CODES = ['COMMUNITY', 'PROFESSIONAL', 'ENTERPRISE', 'ULTIMATE']
...
function openIssue() {
  form.appVersionRange = form.appVersionRange || editions.value?.defaultVersionRange || '>=1.0.0 <2.0.0'
  resetCaps()
...
async function submitIssue() {
  if (!form.tenantCode || !form.customer) { ElMessage.warning(t('cmrule.required')); return }
  issuing.value = true
  try {
    const d = await issueCmruleLicense({
    ...
    ElMessage.success(t('cmrule.issued', { id: d.licenseId }))
    issueOpen.value = false
    await load()
    await loadAudit()
  } catch (e: any) {
    ElMessage.error(e?.response?.data?.message || t('cmrule.issueFail'))
...
async function onRevoke(row: LicenseView) {
  try {
    const { value } = await ElMessageBox.prompt(t('lic.revokeTip'), t('lic.revoke'), {
      confirmButtonText: t('lic.revoke'), type: 'warning', inputValue: t('cmrule.revokeReason'),
    })
...
// ---------- 审计查询 ----------
const ACTIONS = ['CMRULE_LICENSE_ISSUE', 'LICENSE_ISSUE', 'LICENSE_RENEW', 'LICENSE_MODIFY', 'LICENSE_REVOKE', 'LICENSE_EXPIRED']
const audit = reactive({ action: '', keyword: '', range: [] as string[], page: 1, size: 20 })
const auditRows = ref<CmruleAuditRow[]>([])
const auditTotal = ref(0)
const auditLoading = ref(false)
async function loadAudit() {
  if (!auth.has('cmrule:audit')) return
  auditLoading.value = true
  try {
    const p = await queryCmruleAudit({
...
function auditCsv() {
  ...
  downloadFile(`/cmrule/audit/export.csv?${q.toString()}`, 'cmrule-audit.csv')
}
...
function actionColor(action: string) {
  ...
  if (action.startsWith('CMRULE')) return 's-active'
  return 's-soon'
}
</script>
...
<template>
  <div class="wrap">
    <PageHelp id="cmrule" :title="t('help.cmrule.title')"
      :tips="[t('help.cmrule.t1'), t('help.cmrule.t2'), t('help.cmrule.t3')]" />
    <div class="section-title">
      <div>
        <h2>{{ t('nav.cmrule') }}</h2>
        <div class="sub" style="margin-top:.3rem">{{ t('cmrule.lead') }}</div>
      </div>
      <el-button v-if="auth.has('cmrule:issue')" type="primary" size="large" @click="openIssue">{{ t('cmrule.issue') }}</el-button>
    </div>
    ...
    <div class="card" style="margin-bottom:1.1rem">
      <div class="card-h">
        <b>{{ t('cmrule.matrix') }}</b>
        <span class="faint" style="font-size:.78rem">{{ t('cmrule.matrixSub') }}</span>
      </div>
      <table v-if="editions" class="captab">
        <thead>
          <tr>
            <th>{{ t('cmrule.capability') }}</th>
            <th v-for="e in EDITION_CODES" :key="e">{{ t('cmrule.ed.' + e) }}</th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="k in gatedKeys" :key="k">
            <td class="k"><span class="data">{{ k }}</span><span class="faint" style="margin-left:.5rem">{{ t('cmrule.cap.' + k) }}</span></td>
            <td v-for="e in EDITION_CODES" :key="e" class="c" :class="capOf(e, k) ? 'on' : 'off'">{{ capOf(e, k) ? '✓' : '—' }}</td>
          </tr>
        </tbody>
      </table>
      <div class="faint" style="font-size:.76rem;margin-top:.5rem">{{ t('cmrule.matrixNote') }}</div>
    </div>

    <!-- CmRuleEngine License 列表 -->
    <div class="card" style="margin-bottom:1.1rem">
      <div class="card-h"><b>{{ t('cmrule.licenses') }}</b></div>
      <el-table :data="rows" v-loading="loading" style="width:100%">
        ...
        <el-table-column :label="t('lic.edition')" width="140">
          <template #default="{ row }">{{ t('cmrule.ed.' + row.edition) !== 'cmrule.ed.' + row.edition ? t('cmrule.ed.' + row.edition) : row.edition }}</template>
        </el-table-column>
      ...
      <div v-if="!loading && !rows.length" class="faint" style="text-align:center;padding:2rem 0">{{ t('cmrule.empty') }}</div>
    </div>

    <!-- 审计查询 -->
    <div v-if="auth.has('cmrule:audit')" class="card">
      <div class="card-h">
        <b>{{ t('cmrule.auditTitle') }}</b>
        <span class="faint" style="font-size:.78rem">{{ t('cmrule.auditSub') }}</span>
      </div>
      <div class="audit-filters">
        <el-select v-model="audit.action" clearable :placeholder="t('audit.action')" style="width:230px">
          <el-option v-for="a in ACTIONS" :key="a" :label="a" :value="a" />
        </el-select>
        <el-input v-model="audit.keyword" :placeholder="t('cmrule.auditKw')" clearable style="width:240px" @keyup.enter="searchAudit" />
        ...
        <el-button type="primary" @click="searchAudit">{{ t('cmrule.auditQuery') }}</el-button>
    ...
    <!-- 签发向导 -->
    <el-dialog v-model="issueOpen" :title="t('cmrule.issueTitle')" width="760px" align-center>
      <el-form label-position="top">
        ...
        <el-form-item :label="t('lic.edition')">
          <el-radio-group v-model="form.edition" @change="resetCaps">
            <el-radio-button v-for="e in EDITION_CODES" :key="e" :value="e">{{ t('cmrule.ed.' + e) }}</el-radio-button>
          </el-radio-group>
        </el-form-item>
        <el-form-item>
          <template #label>{{ t('cmrule.overrides') }}<HelpTip :content="t('cmrule.tip.overrides')" /></template>
          <div class="capgrid">
            <label v-for="k in gatedKeys" :key="k" class="capitem">
              ...
              <span class="faint" style="font-size:.74rem">{{ t('cmrule.cap.' + k) }}</span>
            </label>
          </div>
        </el-form-item>
        <div class="two">
          ...
          <el-form-item>
            <template #label>{{ t('lic.range') }}<HelpTip :content="t('cmrule.tip.range')" /></template>
        ...
        <div class="notice">🔏 <div>{{ t('cmrule.signNote') }}</div></div>
      </el-form>
      <template #footer>
