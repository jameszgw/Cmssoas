export default {
  ...
  nav: {
    ...
    cmprint: 'CmPrint', tpl: 'Templates',
  },
  tpl: {
    lead: 'CmPrint cloud template library + approval flow: draft → submit (read-only) → approve to publish; versioned with rollback; per-tenant distribution via gallery keys.',
    create: '＋ New template', createTitle: 'New Template', editTitle: 'Edit Template',
    code: 'Code', name: 'Name', tenant: 'Owner', public: 'Public', useCount: 'Used',
    tenantOpt: 'Tenant (empty = platform public)', tenantPh: 'e.g. T-100482', tags: 'Tags (comma separated)', tagsPh: 'delivery,label',
    content: 'Template JSON (with panels)', nameRequired: 'Name is required', badJson: 'Content is not valid JSON',
    created: 'Created {code} (draft)', dirty: 'draft changed',
    search: 'Search code / name / tenant / tags', empty: 'No templates yet.',
    submit: 'Submit', submitTip: 'Template becomes read-only until reviewed. Optional note:', submitted: 'Submitted for approval',
    approve: 'Approve', approveTip: 'The version goes live immediately (enters the gallery). Optional note:', approved: 'Approved & published',
    reject: 'Reject', rejectTip: 'Returns to editable state (live version unchanged). Reason:', rejected: 'Rejected',
    rollback: 'Rollback', rollbackTip: 'Pull v{v} content back into the draft (requires re-approval). Continue?', rolledBack: 'Rolled back to draft — submit for approval',
    export: 'Export', del: 'Delete', deleteTip: 'Delete “{name}” and all its versions permanently?', deleted: 'Deleted',
    disable: 'Unpublish', disableTip: 'It disappears from the gallery (data kept). Continue?', disabled: 'Unpublished',
    enable: 'Publish', enabled: 'Published',
    verTitle: 'Version trail', verEmpty: 'No versions yet (never submitted).', submittedBy: 'Submitted by', reviewedBy: 'Reviewed by',
    st: { DRAFT: 'Draft', PENDING: 'In review', APPROVED: 'Live', DISABLED: 'Unpublished' },
    vst: { PENDING: 'Pending', APPROVED: 'Live', REJECTED: 'Rejected' },
    keysTitle: 'Gallery keys (CmPrint cloudBaseUrl)', keysSub: 'Point the designer cloud-base-url at the URL below',
    keysEmpty: 'No gallery yet. Enter a tenant code (or leave empty = PUBLIC) to generate a key.',
    keyTenantPh: 'Tenant code (empty = PUBLIC)', keyCreate: 'Generate key', keyCreated: 'Gallery enabled for {t}',
    keyReset: 'Reset', keyResetTip: 'The old URL becomes invalid immediately. Continue?', keyResetOk: 'Key reset',
    keysNote: 'A tenant key sees “platform public + own tenant” live templates; designer uploads land as “in review” and only enter the gallery after approval.',
  },
  cmprint: {
    ...
    issue: '＋ Issue CmPrint license', issueTitle: 'Issue CmPrint License', issued: 'Issued {id}',
    issueFail: 'Issue failed', loadFail: 'Failed to load CmPrint licenses', required: 'Tenant ID and customer are required',
    licenses: 'CmPrint Licenses', empty: 'No CmPrint licenses yet.',
    matrix: 'Editions × capability matrix', matrixSub: 'Keys map 1:1 to CmPrint capabilities',
    matrixNote: 'Only keys that differ across editions are listed; all other capabilities (canvas editing, preview, browser printing, template JSON import/export, …) are included in every edition.',
    capability: 'Capability', overrides: 'Capability overrides (contract-level)',
    revokeReason: 'Contract terminated',
    auditTitle: 'Audit query', auditSub: 'Full CmPrint licensing trail: issue / renew / modify / revoke / expire',
    auditKw: 'Search license ID / customer / actor', auditQuery: 'Search',
    signNote: 'The server signs the license with its Ed25519 private key and produces a .lic; claims.features freezes “edition preset ∪ overrides”, so the integrator calls resolveEdition(edition, features) for identical capability gating.',
    ed: { COMMUNITY: 'Community', PROFESSIONAL: 'Professional', ENTERPRISE: 'Enterprise' },
    cap: {
      exportPdf: 'PDF export (raster)', exportPdfText: 'PDF export (text layer)', sharePdf: 'PDF share',
      exportWord: 'Word export', exportExcel: 'Excel/CSV export', exportImage: 'Image export',
      directPrint: 'Silent direct print', overprint: 'Overprint background', calibration: 'Print calibration',
      watermark: 'Watermark', subTemplate: 'Sub-templates', templateGallery: 'Template gallery', theme: 'Designer theming',
    },
    tip: {
  ...
  billing: {
    ...
    collect: 'Collect', scanTitle: 'Scan to pay', scanHint: 'Customer pays by QR; finance verifies receipt, then click Confirm received',
    waiting: 'Awaiting confirmation…', payOk: 'Receipt confirmed', simulatePay: 'Confirm received (manual)',
  ...
  users: {
    ...
    t1: 'Creating a user requires a role; the default initial password is Codeman@123 (customizable).',
    ...
    create: '＋ New user', role: 'Role', initPwd: 'Initial password', initPwdHint: 'blank = default Codeman@123',
  ...
  help: {
    ...
    tpl: {
      title: 'Guide · Template Assets',
      t1: 'Templates follow “draft → submit (read-only) → approve to publish”: edit/submit needs the Edit permission, approve/reject needs the Approve permission — view/edit/use/delete/export are assigned per role.',
      t2: 'Each submission archives a version (submitter / reviewer / notes traceable); “Rollback” pulls an old version into the draft and goes through approval again — the trail is never lost.',
      t3: '“Gallery keys” produce the CmPrint cloud-base-url: a tenant key sees public + own templates; designer uploads require approval before entering the gallery.',
    },
    cmprint: {
  ...
  mail: {
    ...
    subject: '[CODEMAN] Your tenant is ready — activate your admin account',
    welcome: 'Welcome to CODEMAN', tagline: 'Your licensing operations tenant is ready',
  ...
  lic: {
    ...
    crl: 'CRL', autoExpire: 'Auto-expire', copied: 'Copied',
    autoExpireConfirm: 'Mark overdue active licenses as expired and re-sign. Continue?',
    autoExpireOk: 'Expired {n} overdue license(s)',
    crlTitle: 'Revocation List (CRL) distribution', crlLead: 'Signed revocation list; offline clients pull it and verify with the public key (by kid) to reject revoked licenses.',
    crlCount: 'Revoked', crlAlg: 'Signature alg', crlIssuedAt: 'Issued at', crlSig: 'Signature', crlEmpty: 'No revoked licenses.',
    crlPubUrl: 'Public CRL (offline pull)', crlKeysUrl: 'Public keys (JWKS)',
  ...
  cs: {
    ...
    title: 'AI Support', greeting: 'Hi! I am the CODEMAN AI assistant for product, licensing, subscription and billing questions. How can I help?',
