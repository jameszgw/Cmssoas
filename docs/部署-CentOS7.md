...
## 2. 中间件
- **PostgreSQL**：建库 `codeman`、账号与密码;首次启动后端会自动执行 Flyway 迁移(V1–V8)。
- **Redis**：用于心跳 nonce 防重放(多实例水平扩展)。
- **Nacos**：配置中心,创建命名空间(dev/test/prod)与配置 `codeman.yml`,放置 DB/Redis/SMTP/JWT 密钥/`app.license.sign-algo` 等。
...
sudo bash deploy/install.sh \
  ...
  web/console/dist
# 编辑环境变量后重启
sudo vi /etc/codeman/backend.env      # 见 deploy/codeman-backend.env.example
sudo systemctl restart codeman-backend
curl -s http://127.0.0.1:8080/actuator/health
```
- 后端：`systemd` 单元 `codeman-backend`(`deploy/systemd/`),环境变量 `/etc/codeman/backend.env`。
- 前端：nginx 托管 `dist` + 反代 `/api`、`/sdk`(`deploy/nginx/codeman.conf`)。
...
java -jar app.jar
```
配置优先级：Nacos `codeman.yml` > 本地 `application-prod.yml` > `application.yml`。敏感项(密钥/密码)建议只放 Nacos 并加密。
...
- 同一服务起多实例(不同 `--server.port` / 不同主机),nginx `upstream` 或网关按服务名负载均衡：
  ```nginx
  upstream codeman_backend { server 10.0.0.11:8080; server 10.0.0.12:8080; }
  # location /api/ { proxy_pass http://codeman_backend; }
  ```
